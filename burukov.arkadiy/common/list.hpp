...
namespace burukov
{
  ...
  struct Node
  {
    T val_;
    Node< T > *next_;
    Node(const T &value, Node< T > *next) : val_(value), next_(next) {}
    Node(T &&value, Node< T > *next) : val_(std::move(value)), next_(next) {}
  };
  ...
  public:
    LIter() : ptr_(nullptr) {}
    explicit LIter(Node< T > *p) : ptr_(p) {}
  ...
  public:
    LCIter() : ptr_(nullptr) {}
    explicit LCIter(const Node< T > *p) : ptr_(p) {}
  ...
  public:
    ...
    List(const List< T > &other) : head_(nullptr), size_(0)
    {
    ...
    List(List< T > &&other) : head_(other.head_), size_(other.size_)
    {
      other.head_ = nullptr;
      other.size_ = 0;
    }
    ...
    {
      Node< T > *created = new Node< T >(std::move(val), pos.ptr_->next_);
      pos.ptr_->next_ = created;
      ++size_;
      return LIter< T >(created);
    }
    ...
    void pushBack(T &&val)
    {
      Node<T> *nn = new Node<T>(std::move(val), nullptr);
      if (!head_)
        head_ = nn;
      else
      {
