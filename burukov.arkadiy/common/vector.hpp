...
template< class T >
burukov::Vector< T >::Vector(const Vector< T > &rhs) : Vector(rhs.getCapacity())
{
  size_t i = 0;
  for (; i < rhs.getSize(); ++i)
...
template< class T >
burukov::Vector< T >::Vector(Vector< T > &&rhs) noexcept
...
template< class T >
burukov::Vector< T > &burukov::Vector< T >::operator=(const Vector< T > &rhs)
{
...
template< class T >
burukov::Vector< T > &burukov::Vector< T >::operator=(Vector< T > &&rhs) noexcept
{
  if (this == std::addressof(rhs))
    return *this;
...
template< class T >
void burukov::Vector< T >::swap(Vector< T > &rhs) noexcept
{
...
template< class T >
bool burukov::Vector< T >::isEmpty() const noexcept { return !size_; }

template< class T >
size_t burukov::Vector< T >::getSize() const noexcept { return size_; }

template< class T >
size_t burukov::Vector< T >::getCapacity() const noexcept { return capacity_; }


template< class T >
T &burukov::Vector< T >::operator[](size_t id) noexcept
{
...
template< class T >
const T &burukov::Vector< T >::operator[](size_t id) const noexcept
{
  return data_[id];
}

template< class T >
T &burukov::Vector< T >::at(size_t id)
{
  const Vector< T > *cthis = this;
...
template< class T >
const T &burukov::Vector< T >::at(size_t id) const
{
  if (id < getSize())
    return (*this)[id];
...
// ========== Grow ==========
template< class T >
void burukov::Vector< T >::grow(size_t newCap)
{
...
template< class T >
void burukov::Vector< T >::pushBack(const T &v)
{
  Vector< T > t = *this;
...
template< class T >
void burukov::Vector< T >::insert(size_t i, const T &v)
{
  if (i > size_)
...
template< class T >
void burukov::Vector< T >::insert(size_t i, const Vector< T > &rhs, size_t start, size_t end)
{
  if (i > size_)
    throw std::out_of_range("Index out of range");
  if (end > rhs.getSize())
...
template< class T >
template< class P >
void burukov::Vector< T >::uniInsert(size_t i, P &&v)
{
  if (i > size_)
...
template< class T >
void burukov::Vector< T >::erase(size_t i)
{
  if (i >= size_)
    throw std::out_of_range("Index out of range");
  if (i == size_ - 1)
...
template< class T >
void burukov::Vector< T >::insert(CIter< T > it, const T &v)
{
...
template< class T >
void burukov::Vector< T >::insert(CIter< T > it, T &&v)
{
  if (it < CIter< T >(data_) || it > CIter< T >(data_ + size_))
    throw std::out_of_range("position out of range");
  size_t pos = it - CIter< T >(data_);
...
template< class T >
void burukov::Vector< T >::insert(CIter< T > i, CIter< T > start, CIter< T > end)
{
...
template< class T >
burukov::CIter< T > burukov::Vector< T >::erase(CIter< T > i)
{
  if (i < CIter< T >(data_) || i >= CIter< T >(data_ + size_))
...
template< class T >
template< class C >
burukov::CIter< T > burukov::Vector< T >::erase(CIter< T > start, CIter< T > end, C c)
{
  if (start < CIter< T >(data_) || end > CIter< T >(data_ + size_) || start > end)
...
template< class T >
void burukov::Vector< T >::reserve(size_t k)
{
...
template< class T >
void burukov::Vector< T >::pushBackCount(size_t k, const T &v)
{
  if (!k) return;
  Vector< T > t = *this;
...
template< class T >
template< class IT >
void burukov::Vector< T >::pushBackRange(IT b, size_t c)
{
  if (!c) return;
  Vector< T > t = *this;
...
bool burukov::operator==(const Vector< T > &lhs, const Vector< T > &rhs)
{
  ...
  for (size_t i = 0; i < lhs.getSize() && isEqual; ++i)
